...
export const RULE_NAME = 'prefer-describe-function-name'
...
export default createEslintRule<Options, MESSAGE_IDS>({
  ...
  meta: {
    ...
    docs: {
      description:
        'enforce using a function as a describe name over an equivalent string',
      recommended: false
    },
    fixable: 'code',
    schema: [],
  ...
  create(context) {
    return {
      CallExpression(node) {
        ...
        const describedName = argument.value
        if (!context.filename.includes(`${describedName}.`)) {
          return
        }
        ...
        const scopedFunction = scope?.set.get(describedName)?.defs[0]
        ...
        context.report({
          ...
          fix(fixer) {
            return fixer.replaceText(argument, describedName)
          }
        })
      }
    }
